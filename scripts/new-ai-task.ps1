...
- 运行测试、构建、类型检查、lint 或其他适合本项目的验证命令。
...
- 未引入明显无关改动。
- worker 总结已运行的验证命令及其结果；如果失败，说明失败原因和后续建议。

## 建议验证命令

- 未由用户指定。worker 应根据项目 manifest、脚本和文档选择合适的验证命令运行。

## 交付物要求

- 在当前工作区留下未提交修改，供用户人工核查并最终确认。
- 在 worker 总结中列出修改内容、验证命令、验证结果、未完成事项或 blocker。
"@
